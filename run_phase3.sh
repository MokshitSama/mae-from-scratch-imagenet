#!/usr/bin/env bash
# Phase 3 (MAE finetune) - accelerate DDP on the six 5090s (PCI 0,1,2,3,5,6; index 4 is the 3090, EXCLUDED).
...
export V="${1:-${V:-1}}"         # run version from the command line: bash run_phase3.sh 3  (or V=3 bash run_phase3.sh)
...
cd "$(dirname "$0")/phase3"
accelerate launch --multi_gpu --num_processes 6 --gpu_ids 0,1,2,3,5,6 --main_process_port 29503 train.py
