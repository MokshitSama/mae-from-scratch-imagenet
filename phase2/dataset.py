...
mae_transforms = A.Compose([
    A.RandomResizedCrop(size=(CFG.img_size[0], CFG.img_size[1]), scale=(0.2, 1.0)),  # 0.2, not 0.08
    A.HorizontalFlip(p=0.5),
    A.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD),
    ToTensorV2(),
])
...
def build_mae_loader():
    paths, labels, _ = build_index(os.path.join(CFG.data_root, "train"))
    ds = IMAGENET_DATASET(paths, labels, transforms=mae_transforms)
    loader = DataLoader(ds, batch_size=CFG.batch_size, shuffle=True,
                    num_workers=CFG.num_workers, pin_memory=True,      # False -> True  (the +25% overlap fix)
                    drop_last=True, persistent_workers=CFG.num_workers > 0,
                    prefetch_factor=4)                                 # add: keep the pipeline full (default was 2)
