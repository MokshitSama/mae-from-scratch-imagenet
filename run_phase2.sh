#!/usr/bin/env bash
# Phase 2 (MAE pretrain) - accelerate DDP on the six 5090s (PCI 0,1,2,3,5,6; index 4 is the 3090, EXCLUDED).
...
#       is installed and torch's RPATH already resolves to it; the preload below is belt-and-suspenders.
set -e
...
cd "$(dirname "$0")/phase2"
accelerate launch --multi_gpu --num_processes 6 --gpu_ids 0,1,2,3,5,6 --main_process_port 29502 train_mae.py
